...
class BancoController extends Controller
{
    ...
    {
        $bancos = Banco::all();
        return view('admin.bancos.home', compact('bancos'));
    }

    public function getBancos()
    {
        $bancos = Banco::select(['id', 'imagem', 'titulo', 'created_at', 'updated_at'])->orderBy('created_at', 'desc');

        return DataTables::of($bancos)
            ->addColumn('action', function($query) { 
                return $botoes ="<div style='display: flex; justify-content: center;'><a href='". route('admin.bancos.edit', $query->id) ."' class='btn btn-primary btn-sm me-1'>Editar</a>
                    ...
                    <div class='modal-content modal-filled bg-danger'>
                        <div class='modal-body p-4'>
                            <div class='text-center'>
                                ...
                                <form action='".route('admin.bancos.delete', $query->id)."' method='POST'>
                                ". method_field('DELETE') ."
                                ". csrf_field() ."
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasFile('bg_imagem')) {
            ...
            $banco['bg_imagem'] = $pathBgImagem;
        }

        // Criar o registro no banco de dados
        $banco = Banco::create($banco);

        return redirect()->route('admin.bancos.home')->with('success', 'Post cadastrado com sucesso!');
    }
     ...
     */
    public function edit(Banco $banco, string|int $id)
    {
        if (!$banco = Banco::find($id)) {
     ...
     */
    public function update(Request $request, Banco $banco, string $id)
    {
        if (!$banco = $banco->find($id)) {
            return redirect()->back();
        }
        ...
        if ($request->has('delete_imagem') && $banco->imagem) {
            Storage::delete($banco->imagem);
            $banco->imagem = null;
        }

        // Excluir imagem de fundo, se solicitado
        if ($request->has('delete_bg_imagem') && $banco->bg_imagem) {
            Storage::delete($banco->bg_imagem);
            $banco->bg_imagem = null;
        }

        // Atualizar os dados da notícia, exceto as imagens
        $banco->update($request->except(['imagem', 'bg_imagem']));
        ...
        if ($request->hasFile('bg_imagem')) {
            ...
            $banco->bg_imagem = $path;
        }

        $banco->save();

        return redirect()->route('admin.bancos.home')->with('success', 'Post atualizada com sucesso.');
    }
    ...
    public function destroy($id)
    {
        ...
        if ($banco) {
            ...
            $banco->delete();
            return redirect()->route('admin.bancos.home')->with('success', 'Post excluído com sucesso!');
        } else {
            // Se a notícia não for encontrada, redirecionar de volta com uma mensagem de erro
            return redirect()->route('admin.bancos.home')->with('error', 'Erro ao excluir post. Post não encontrado.');
        }
    }
}
